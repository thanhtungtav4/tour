...
export function Footer() {
  ...
  return (
    <footer className="bg-[#0e1425] text-white">
      {/* Main Footer */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8 lg:gap-6">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="flex gap-3 mb-6">
              <a href="#" className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-blue-600 transition-colors" aria-label="Facebook">
              ...
              <a href="https://zalo.me/0928382087" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-blue-500 transition-colors" aria-label="Zalo">
            ...
            {/* Newsletter */}
            <div>
              <h4 className="text-sm font-semibold text-white mb-2">Đăng ký nhận ưu đãi</h4>
              <form onSubmit={handleSubscribe} className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-sm text-white placeholder-gray-400 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500"
                  required
                />
                <button
                  type="submit"
              ...
              {subscribed && (
                <p className="mt-1 text-xs text-emerald-400">Đã đăng ký thành công!</p>
              )}
            </div>
          </div>
          ...
          {/* Popular Tours */}
          <div>
            <h4 className="text-sm font-semibold text-white mb-4">Tour phổ biến</h4>
                ...
                <li key={tour.id}>
                  <Link href={`/booking/${tour.slug}`} className="group flex items-center gap-2">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-white truncate group-hover:text-emerald-400 transition-colors">{tour.name}</p>
                      <p className="text-xs text-gray-400">{tour.price.toLocaleString("vi-VN")}đ</p>
                    </div>
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-2">
              {policies.map((policy) => (
                <li key={policy.href}>
                  <Link href={policy.href} className="text-sm text-gray-400 hover:text-emerald-400 transition-colors">
                    {policy.label}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-3">
              <li className="flex items-start gap-2">
                <PhoneIcon className="w-4 h-4 text-emerald-400 mt-0.5 flex-shrink-0" />
                <div>
              ...
              <li className="flex items-start gap-2">
                <MailIcon className="w-4 h-4 text-emerald-400 mt-0.5 flex-shrink-0" />
                <a href="mailto:doidepadventure@gmail.com" className="text-sm text-gray-400 hover:text-emerald-400 transition-colors break-all">
            ...
            <div className="mt-6">
              ...
              <div className="flex gap-2">
                ...
                <div className="px-2 py-1 bg-white/10 rounded text-xs text-gray-400">Tiền mặt</div>
              </div>
            </div>
          </div>
        </div>
